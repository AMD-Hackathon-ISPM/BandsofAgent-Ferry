...
const VERB: Record<MessageType, string> = {
  finding: "Cargo inspected at",
  task_request: "Work order issued to",
  handoff: "Cargo handed to",
  review: "Audit performed at",
  decision: "Clearance decided at",
  artifact_created: "Container produced at",
  blocker: "Shipment held at",
}
    ...
    >
      <div className="pointer-events-auto overflow-hidden rounded-md border border-border bg-card/85 shadow-lg backdrop-blur-sm">
        <div className="flex items-center justify-between border-b border-border px-3 py-1.5">
          ...
          <span className="dot-pulse size-1.5 rounded-full bg-signal" aria-hidden="true" />
        </div>
        {recent.length === 0 ? (
          <p className="px-3 py-3 text-[11px] text-muted-foreground">
            No harbor events yet.
          </p>
        ) : (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-[11px] leading-tight">
                      <span className="text-muted-foreground">{VERB[m.type]} </span>
                  ...
                  <time className="shrink-0 text-[10px] text-muted-foreground/70" dateTime={m.createdAt}>
